...
gpkg_file = "C:\\Users\\user\\Documents\\MBA\\geoviz\\Aula3\\car_embargos.gpkg"

# Informar a coluna com a matrícula do imóvel rural
coluna_matricula_imovel = 'cod_imovel'

# ========== CACHE DOS DADOS ==========
# Esta função roda APENAS na primeira vez
@st.cache_data
def carregar_dados_iniciais(gpkg_file):
    area = ler_geodataframe(gpkg_file, "area_imovel")
    ibama = ler_geodataframe(gpkg_file, "embargos_ibama")
    icmbio = ler_geodataframe(gpkg_file, "embargos_icmbio")
    return area, ibama, icmbio

area_imovel, embargos_ibama, embargos_icmbio = carregar_dados_iniciais(gpkg_file)

# Selecionar o códigos dos imóveis do CAR
